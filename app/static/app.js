...
function updateStudentIdField() { const needsStudentId = ["student", "parent"].includes($("username").value.trim().toLowerCase()); $("studentIdField").classList.toggle("hidden", !needsStudentId); $("loginStudentId").required = needsStudentId; }
$("username").addEventListener("input", updateStudentIdField);
$("loginForm").addEventListener("submit", async e => { e.preventDefault(); try { state.user = await request("/auth/login", {method:"POST", body:JSON.stringify({username:$("username").value, password:$("password").value, student_id:$("loginStudentId").value || null})}); sessionStorage.setItem("campus_token", state.user.access_token); sessionStorage.setItem("campus_user", JSON.stringify(state.user)); await enterApp(); } catch(err) { toast(err.message, true); } });
...
document.querySelectorAll("[data-demo]").forEach(button => button.onclick = () => { const [user,password] = button.dataset.demo.split("|"); $("username").value=user; $("password").value=password; $("loginStudentId").value = ""; updateStudentIdField(); if (["student", "parent"].includes(user)) $("loginStudentId").focus(); });
